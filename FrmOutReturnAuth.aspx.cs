...
public partial class FrmOutReturnAuth : System.Web.UI.Page
{
    ...
    {
        CLSOUTRETURNAUTH.Getinfotable(grdOwgData, Session["MID"].ToString(), Session["BRCD"].ToString(),Session["EntryDate"].ToString(),"O","OR");
    }
    ...
    protected void lnkEdit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string url = "FrmOutAuthDo.aspx" + "?setno=" + setscroll[0] + "&scrollno=" + setscroll[1] + "&op=returnauth";
                NewWindows(url);
            }
            else
            {
