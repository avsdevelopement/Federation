...
public class ClsDDCR
{
    ...
    public int CloseDDS(string AGNO, string ACNO, string BRCD, string ENTRYDATE, string MID, bool PF,string ParaUDT="0")
    {
        try
        {
            try
    ...
    public int CloseLoanAcc(string BRCD, string SubglCode, string AccNo, string EDate, string Mid)
    {
        try
        {
            ...
            sql = "Update Avs_LienMarkDetails Set Status = 3, ClosingDate= '" + conn.ConvertDate(EDate).ToString() + "', Stage = 1004  Where BRCD = '" + BRCD + "' AND LOANGLCODE = '" + SubglCode + "' AND LoanAccNo = '" + AccNo + "'";
            Result = conn.sExecuteQuery(sql);
    ...
    public DataTable GetStageSetno(string Brcd,string Subgl,string Edate)//Dhanya Shetty//12/06/2018
    {
        try
        {
            string[] TD = Edate.Split('/');
            TableName = "AVSM_" + TD[2].ToString() + TD[1].ToString();
            sql = "select DISTINCT(Setno),stage from " + TableName + " where brcd='" + Brcd + "' and subglcode='" + Subgl + "' and entrydate='" + conn.ConvertDate(Edate) + "' and stage<>1004";
            DT = conn.GetDatatable(sql);
        }
        catch (Exception Ex)
        {
    ...
    public string GetAgName_Code(string BRCD, string Accno)
    {
        try
        {
            sql = "Select ConVert(VarChar(10), ConVert(BigInt, A.SubGlCode))+'_'+C.GLNAME+'_'+ ConVert(VarChar(10), ConVert(BigInt, A.CustNo)) Name " +
                    " from AVS_ACC				A With(NoLock) " +
                    " inner join AGENTMAST 		B With(NoLock)			on B.BRCD=A.BRCD and B.AGENTCODE=A.SUBGLCODE " +
                    " inner join GLMAST			C With(NoLock)			on C.BRCD=A.BRCD and C.SUBGLCODE=A.SUBGLCODE " +
                    " Where A.BrCd = '" + BRCD + "' And A.Glcode = '2' And A.Stage <> 1004 and A.ACCNO=" + Accno + "";
            sql = conn.sExecuteScalar(sql);
