...
/// <summary>
/// Summary description for ClcMultiPosting
/// </summary>
public class ClcMultiPosting
{
    ...
    public int InsertData(string AG, string BRCD, string MID, string EDT)
    {
        try
        {
            sql = "insert into AFTEKR(ENTRYDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE)" +
                "select '" + conn.ConvertDate(EDT) + "','0',SUBGLCODE,BRCD,ACCNO,'1001','" + MID + "',GETDATE() from AVS_ACC where GLCODE=2 and SUBGLCODE='" + AG + "' and BRCD='" + BRCD + "' and ACC_STATUS in (1,4)";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int UpdateAFTData(string AG, string EDT, string PDT, string AC, string BRCD, string AMT, string TDT, string FL,string OLDAMT)
    {
        try
        {
            ...
            else if (FL == "CHGAMT")
            {
                ...
                sql = "UPDATE AFTEKR SET TRANSAMT='" + AMT + "' where brcd='" + BRCD + "' and entrydate ='" + conn.ConvertDate(EDT) + "' and AGENTCODE='" + AG + "' and TRANSAMT='" + OLDAMT + "' and acno='" + AC + "'";
            }
    ...
    public string GetAgentColMob(string AG, string BRCD)
    {
        try
        {
            sql = "Select sum(TRANSAMT) from  AFTEKR where Ref_Agent='" + AG + "' and brcd='" + BRCD + "'";
            BRCD = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
    ...
    public string GetAmtInfoMOb(string AG, string AC, string EDT, string BRCD,string amount)
    {
        try
        {
            sql = "SELECT TRANSAMT FROM AFTEKR where ENTRYDATE='" + conn.ConvertDate(EDT) + "' and AGENTCODE='" + AG.Trim() + "' AND  BRCD ='" + BRCD.Trim() + "' AND ACNO='" + AC.Trim() + "' and TRANSAMT='" + amount.Trim()+ "'";
            BRCD = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int InsertData_Mob(string EDT, string BRCD, string SBGL, string GL)
    {
        try
        {
            if (GL == "2")
            {
                sql = "insert into AFTEKR(ENTRYDATE,POSTINGDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE,Ref_Agent,Glcode) " +
                    " select Convert(varchar(11),ENTRYDATE,120)ENTRYDATE,Convert(varchar(11),POSTINGDATE,120)POSTINGDATE,CREDIT,SUBGLCODE,BRCD,ACCNO,'1001','" + BRCD + "',GETDATE(),Ref_Agent,Glcode from ALLVCR " +
            ...
            else if (GL == "5")
            {
                sql = "insert into AFTEKR(ENTRYDATE,POSTINGDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE,Ref_Agent,Glcode) " +
                        " select Convert(varchar(11),ENTRYDATE,120)ENTRYDATE,Convert(varchar(11),POSTINGDATE,120)POSTINGDATE,CREDIT,SUBGLCODE,BRCD,ACCNO,'1001','" + BRCD + "',GETDATE(),Ref_Agent,Glcode from ALLVCR " +
                        " where GLCODE=5  and BRCD='" + BRCD + "' AND STAGE IN (1001,1002) and pmtmode='MOB-COL' and Ref_agent='" + SBGL + "'"; //ASHOK MISAL AND STAGE NOT IN (1005,1003)  CHANGES
            }
            else if (GL == "1")
            {
                sql = "insert into AFTEKR(ENTRYDATE,POSTINGDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE,Ref_Agent,Glcode) " +
                    " select Convert(varchar(11),ENTRYDATE,120)ENTRYDATE,Convert(varchar(11),POSTINGDATE,120)POSTINGDATE,CREDIT,SUBGLCODE,BRCD,ACCNO,'1001','" + BRCD + "',GETDATE(),Ref_Agent,Glcode from ALLVCR " +
                    " where GLCODE=1 and  BRCD='" + BRCD + "' AND STAGE IN (1001,1002) and pmtmode='MOB-COL' and Ref_agent='" + SBGL + "'"; //ASHOK MISAL AND STAGE NOT IN (1005,1003)  CHANGES
            }
            else if (GL == "3")
            {
                sql = "insert into AFTEKR(ENTRYDATE,POSTINGDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE,Ref_Agent,Glcode) " +
                    " select Convert(varchar(11),ENTRYDATE,120)ENTRYDATE,Convert(varchar(11),POSTINGDATE,120)POSTINGDATE,CREDIT,SUBGLCODE,BRCD,ACCNO,'1001','" + BRCD + "',GETDATE(),Ref_Agent,Glcode from ALLVCR " +
                    " where GLCODE=3  and BRCD='" + BRCD + "' AND STAGE IN (1001,1002) and pmtmode='MOB-COL' and Ref_agent='" + SBGL + "'"; //ASHOK MISAL AND STAGE NOT IN (1005,1003)  CHANGES
            }
            else if (GL == "15")
            {
                sql = "insert into AFTEKR(ENTRYDATE,POSTINGDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE,Ref_Agent,Glcode) " +
                    " select Convert(varchar(11),ENTRYDATE,120)ENTRYDATE,Convert(varchar(11),POSTINGDATE,120)POSTINGDATE,CREDIT,SUBGLCODE,BRCD,ACCNO,'1001','" + BRCD + "',GETDATE(),Ref_Agent,Glcode from ALLVCR " +
                    " where GLCODE=15  and BRCD='" + BRCD + "' AND STAGE IN (1001,1002) and pmtmode='MOB-COL' and Ref_agent='" + SBGL + "'"; //ASHOK MISAL AND STAGE NOT IN (1005,1003)  CHANGES
            }
            else
            {
                sql = "insert into AFTEKR(ENTRYDATE,POSTINGDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE,Ref_Agent,Glcode) " +
                        " select Convert(varchar(11),ENTRYDATE,120)ENTRYDATE,Convert(varchar(11),POSTINGDATE,120)POSTINGDATE,CREDIT,SUBGLCODE,BRCD,ACCNO,'1001','" + BRCD + "',GETDATE(),Ref_Agent,Glcode from ALLVCR " +
                        " where  BRCD='" + BRCD + "' AND STAGE IN (1001,1002) and pmtmode='MOB-COL' and Ref_agent='" + SBGL + "'"; //ASHOK MISAL AND STAGE NOT IN (1005,1003)  CHANGES
            }
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        ...
        try
        {
            ...
            else if (FL == "INV")
            {
                sql = "Select ROW_NUMBER () OVER (PARTITION BY SUBGLCODE order by ACCNO) as ID ,'' as NAME,ACCNO,'' as AMOUNT,'A/C not created' REMARK From allvcr Where Brcd='" + BRCD + "' And Subglcode='" + SBGL + "' " +
                    "  And Entrydate='" + conn.ConvertDate(EDT) + "' And Accno Not in (Select Accno From avs_acc Where Brcd='" + BRCD + "' And Subglcode='" + SBGL + "')";
            }
            DT = conn.GetDatatable(sql);
        }
        catch (Exception Ex)
    ...
    public int UpdateALLVCRStage(string BRCD, string SBGL)
    {
        try
        {
            sql = "UPDATE ALLVCR SET STAGE='1003' WHERE PMTMODE='MOB-COL' AND BRCD='" + BRCD + "' AND Ref_agent='" + SBGL + "'";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public DataTable GetPostingData(string AGCD, string BRCD, string EDT, string FL)
    {
        try
        {
            if (FL != "MOB")
            {
                sql = "Select ACNO, TRANSAMT,  TRANSACTIONDATE TRANSACTIONDATE,POSTINGDATE from  AFTEKR where agentcode='" + AGCD + "' and brcd='" + BRCD + "' and ENTRYDATE='" + conn.ConvertDate(EDT) + "' and TRANSAMT>0";
            }
            else
            {
                sql = "Select glcode,AGENTCODE,ACNO, TRANSAMT,  TRANSACTIONDATE TRANSACTIONDATE,POSTINGDATE from  AFTEKR where Ref_Agent='" + AGCD + "' and brcd='" + BRCD + "' and TRANSAMT>0";
            }
            DT = conn.GetDatatable(sql);
    ...
    public DataTable GetDataAllVCR(string agentcode, string BRCD)
    {
        DataTable DT = new DataTable();
        try
        {
            sql = "select * from allvcr where ref_agent='"+agentcode+"' and brcd='"+BRCD+"' and stage=1001 and PMTMODE='MOB-COL'";
            DT = conn.GetDatatable(sql);
        }
        catch (Exception ex)
        {
    ...
    public int UpdateStageALLVCR(string agentcode, string BRCD)
    {
        int Result = 0;
        try
        {
            sql = "delete allvcr where ref_agent='" + agentcode + "' and brcd='" + BRCD + "' and stage=1001 and PMTMODE='MOB-COL'";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception ex)
        {
    ...
    public int UpdateMobileStatus(string agentcode, string BRCD,string BankCD,string Stage)
    {
        int Result = 0;
        try
        {
            sql = "update allvcr_"+BankCD+" set stage="+Stage+" where ref_agent='" + agentcode + "' and brcd='" + BRCD + "' and stage=1001 and status=1";
            Result = conn.sExecuteQueryMob1(sql);
        }
        catch (Exception ex)
        {
            ExceptionLogging.SendErrorToText(ex);
    ...
    public void UpdateMobileStage(string BankCode, string BRCD, string AgentCode)
    {
        try
        {
            sql = "update allvcr_"+BankCode+" set status=2 where ref_agent='"+AgentCode+"' and BRCD='"+BRCD+"' and status=1";
            conn.sExecuteQuery(sql);
        }
        catch (Exception ex)
        {
