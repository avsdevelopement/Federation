...
public class ClsAVS5028
{
    ...
    public int GetFromSurity(GridView grd, string BRCD, string CustNo, string PrCode, string EDate, string Flag)
    {
        try
        {
            sql = "Exec SP_SURITYINFO @BRCD = '" + BRCD + "', @CUSTNO = '" + CustNo + "', @GLCODE = '" + PrCode + "', @EDATE = '" + conn.ConvertDate(EDate) + "', @FLAG = '" + Flag + "'";
   ...
   public int UpdateAccDetails(string BrCode, string GlCode, string PrCode, string CustNo, string EDate, string Mid)  
    {
        try
        {
            sql = "Update AVS_ACC Set OpeningDate = '" + conn.ConvertDate(EDate) + "',CLOSINGDATE=Null,ACC_STATUS=1, Mid = '" + Mid + "', Stage = '1003' "+
                          "Where BrCd = '" + BrCode + "' And GlCode = '" + GlCode + "' And SubGlCode = '" + PrCode + "' And CustNo = '" + CustNo + "' And AccNo = '" + CustNo + "' ";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception ex)
...
public void OldAccClose(string BrCode, string ProdCode, string AccNo, string EDate) // Lmstatus & Acc Status Change 08-05-2019
    {        
        double Balance = 1;
        try
        {
            ...
            if (Balance == 0.00)
            {
                sql = "Update LoanInfo Set LmStatus = '1' Where BrCd = '" + BrCode + "' And LoanGlCode = '" + ProdCode + "' And CustAccNo = '" + AccNo + "' And LmStatus = '1'";
                Result = conn.sExecuteQuery(sql);
                sql = "Update Avs_Acc Set Acc_Status = '1', ClosingDate = '" + conn.ConvertDate(EDate) + "' Where BrCd = '" + BrCode + "' And SubGlCode = '" + ProdCode + "' And AccNo = '" + AccNo + "'";
                Result = conn.sExecuteQuery(sql);
            }
        }
        catch (Exception Ex)
