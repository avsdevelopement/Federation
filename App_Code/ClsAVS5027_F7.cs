...
/// <summary>
/// Summary description for ClsAVS5027_F7
/// </summary>
public class ClsAVS5027_F7
{
    ...
    public string GetSurityStandby(string Member, string CustNo)
    {
        try
        {
            sql = "Select Count(*) as Surity From AvsLnsurityTable Where MemberNo = '" + Member + "' And  CustNo <> '" + CustNo + "' and Stage <> '1004' ";
    ...
    public DataTable GetSurityDetails(string PROD, string CustNo, string BRCD)
    {
        try
        {
            sql = "Select MEM_NO_SURITY As Txtmemno, ACCNO As Txtcustno,SURITYSRNO, SURITYNAME As Txtname, ConVert(VarChar(10), Memberdate, 121) As Memberdate, " +
                  "ConVert(VarChar(10), DATEOFRET, 121) As DATEOFRET, BAOFSURITY As LoanBal, REMSERVICE,0 As StandBy, STANDS " +
    ...
    public DataTable GetLoanAPPCount(string Flag, string BRCD, string CustNo, string FDT, string Mem)
    {
        DataTable dt = new DataTable();
        try
        {
            sql = "exec LoanAPPValidation @Brcd='" + BRCD + "',@AsOnDate='" + conn.ConvertDate(FDT) + "',@CustNo='" + CustNo + "',@Type='" + Flag + "',@Member='" + Mem + "' ";
            dt = conn.GetDatatable(sql);
        }
        catch (Exception ex)
        {
    ...
    public string GetRecTypeCode(string BRCD, string Prd)
    {
        string RES = "";
        try
        {
            sql = "Select Top 1 SURYN from LoanGl " +
                            " where BRCD='" + BRCD + "' " +
                             " and LoanGlcode ='" + Prd + "' " +
                            " order  by LoanGlcode";
            sResult = conn.sExecuteScalar(sql);
    ...
    public string GetRecCount(string BRCD, string Prd)
    {
        string RES = "";
        try
        {
            sql = "Select Top 1 SURNO from LoanGl " +
                            " where BRCD='" + BRCD + "' " +
                             " and LoanGlcode ='" + Prd + "' " +
                            " order  by LoanGlcode";
            sResult = conn.sExecuteScalar(sql);
    ...
    public string GetJointDate (string Custno)
    {
        string RES = "";
        try
        {
            sql = "Select Top 1 Convert(Varchar(10),DOJ,103) As DOJ from EmpDetail " +
                            " where Custno='" + Custno + "' " +
                            " order  by Custno";
            sResult = conn.sExecuteScalar(sql);
    ...
    public string GetRetireDate(string Custno)
    {
        string RES = "";
        try
        {
            sql = "Select Top 1 Convert(Varchar(10),DOR,103) As DOJ from EmpDetail " +
                            " where Custno='" + Custno + "' " +
                            " order  by Custno";
            sResult = conn.sExecuteScalar(sql);
    ...
    public string Validate(string Brcd, string Prd)
    {
        try
        {
            sql = "Select Validate From LoanGl Where Brcd = '" + Brcd + "' And LoanGlcode = '" + Prd + "' ";
            sResult = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int GetCustDelete (string brcd, string stage, string mid, string prd, string Scust)
    {
        try
        {
            sql = "Delete From AVS1003 Where BRCD ='" + brcd + "' And PRODUCT='" + prd + "' And STAGE='" + stage + "' And MID='" + mid + "' And ACCNO='" + Scust + "'   ";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public DataTable GetDedutcionCalc(string Brcd, string Custno, string AsOndate, string Amt)
    {
        DataTable dt = new DataTable();
        try
        {
            sql = "exec Sp_StdDeductionCalc @BrCd='" + Brcd + "',@CustNo='" + Custno + "',@AsonDate='" + conn.ConvertDate(AsOndate) + "',@SanAmount='" + Amt + "' ";
            dt = conn.GetDatatable(sql);
        }
        catch (Exception ex)
        {
