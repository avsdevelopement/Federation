...
public class ClsSharesAllotment
{
    ...
    public string GetMaxMemNo(string BrCode, string CustNo) // Rakesh 07-02-2019 Add Custno 
    {
        try
        {
            string Acc = "";
    ...
    public int UpDateAppStatus(string BrCode, string CustNo, string CertNo, string MemberNo, string ApplStatus, string AppNo, string MemBRCD, string EDate, string Flag, string SetNo)
    {
        try
        {
            sql = "Exec Sp_ShareAppllotment @BrCode = '" + BrCode + "', @CustNo = '" + CustNo + "', @CertNo = '" + CertNo + "', @MemberNo = '" + MemberNo + "', @ApplStatus = '" + ApplStatus + "', @AppNo = '" + AppNo + "', @MemBRCD = '" + MemBRCD + "', @EntryDate = '" + conn.ConvertDate(EDate).ToString() + "', @Flag = '" + Flag + "',@SetNo='" + SetNo + "'";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
