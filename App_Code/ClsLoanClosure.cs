...
public class ClsLoanClosure
{
    ...
    public DataTable GetCustName(string GLCODE, string ACCNO, string BRCD)
    {
        ...
        try
        {
            sql = "SELECT M.CUSTNAME+'_'+CONVERT(VARCHAR(10),AC.CUSTNO) CUSTNAME FROM MASTER M INNER JOIN AVS_ACC AC ON AC.CUSTNO=M.CUSTNO  WHERE AC.ACCNO='" + ACCNO + "' AND AC.SUBGLCODE='" + GLCODE + "' AND AC.BRCD='" + BRCD + "'";   // CHNAGES BY ASHOK AND M.BRCD=AC.BRCD 
    ...
    public DataTable GetAllFieldData(string SubGlCode, string AccNo, string BrCode, string EDate)
    {
        ...
        try
        {
            sql = "Exec RptLoanDetailsInfo @BrCode = '" + BrCode + "', @SubGlCode = '" + SubGlCode + "', @AccNo = '" + AccNo + "', @EDate = '" + conn.ConvertDate(EDate).ToString() + "', @Flag = 'LDIC'";
            DT = conn.GetDatatable(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int UpdateLastIntDate(string BrCode, string SGlCode, string AccNo, string EDate)
    {
        try
        {
    ...
    public int CloseLoanAcc(string BRCD, string SubglCode, string AccNo, string EDate, string Mid)
    {
        try
        {
            sql = "Update LoanInfo Set LmStatus = 99, Prev_IntDt = LastIntDate, LastIntDate = '" + conn.ConvertDate(EDate).ToString() + "', Stage = 1003, Mod_Date = '" + conn.ConvertDate(EDate).ToString() + "' Where BRCD = '" + BRCD + "' AND LOANGLCODE = '" + SubglCode + "' AND CUSTACCNO = '" + AccNo + "'";
            Result = conn.sExecuteQuery(sql);

            if (Convert.ToInt32(Result) > 0)
            {
                sql = "Update Avs_Acc Set Acc_status = 3, Stage = 1003, LastIntDt = '" + conn.ConvertDate(EDate).ToString() + "', CLOSINGDATE = '" + conn.ConvertDate(EDate).ToString() + "' Where BRCD = '" + BRCD + "' AND SUBGLCODE = '" + SubglCode + "' AND ACCNO = '" + AccNo + "'";
                Result = conn.sExecuteQuery(sql);
            }
            ...
            if (Result > 0)
            {
                ...
                if (DTG.Rows.Count > 0)
                {
                    ...
                    {
                        // Added by Abhishek on 13-03-2018 To Close Lien Status
                        sql = "Update AVS_LienMarkDetails Set Status = '3',ClosingDate='" + conn.ConvertDate(EDate).ToString() + "' where BrCd = '" + BRCD + "' And LoanGlCode = '" + SubglCode + "' And LoanAccno = '" + AccNo + "'";
                        conn.sExecuteQuery(sql);

                        sql = "Update Avs_Acc Set Acc_Status = '1' Where BrCd = '" + DTG.Rows[i]["BRCD"].ToString() + "' And SubGlCode = '" + DTG.Rows[i]["DepositGlCode"].ToString() + "' And AccNo = '" + DTG.Rows[i]["DepositAccNo"].ToString() + "'";
                        conn.sExecuteQuery(sql);
                    }
                }
            }
    ...
    public int InsertIntoTable(string BrCode, string CustNo, string GlCode, string SubGlCode, string AccNo, string CustName, string Amount, string TrxType, string Activity, string PmtMode, string Perticulars, string Perticulars2, string ChkNo, string ChkDate, string EDate, string Mid)
    {
        try
        {
            sql = "Insert Into Avs_TempMultiTransfer (BrCd, CustNo, CustName, GlCode, SubGlCode, AccNo, Particulars, Particulars2, Amount, TrxType, Activity, PmtMode, RefId, InstNo, InstDate, EntryDate, SystemDate, Mid) " +
                  "VALUES ('" + BrCode + "','" + CustNo + "','" + CustName + "', '" + GlCode + "','" + SubGlCode + "','" + AccNo + "','" + Perticulars + "','" + Perticulars2 + "','" + Amount + "','" + TrxType + "','" + Activity + "','" + PmtMode + "','0','" + ChkNo + "','" + ChkDate + "', '" + conn.ConvertDate(EDate).ToString() + "', GetDate(), '" + Mid + "')";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int BindGrid(GridView Grd, string BRCD, string GLCD, string SGLCD, string ACC, int FL)
    {
        try
        {
            if (FL == 1)
                sql = "EXEC DDSClousureState @PRDTCD ='" + SGLCD + "',@GLCD='" + GLCD + "',@ACCNO ='" + ACC + "',@BRCD='" + BRCD + "'";
            else
                sql = "EXEC DepositeClousureState @PRDTCD ='" + SGLCD + "',@GLCD='" + GLCD + "',@ACCNO ='" + ACC + "',@BRCD='" + BRCD + "'";
