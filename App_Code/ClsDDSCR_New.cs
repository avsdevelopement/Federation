...
/// <summary>
/// Summary description for ClsDDSCR_New
/// </summary>
public class ClsDDSCR_New
{
    ...
    public DataTable GetInfo(string AGNO, string BRCD, string EDT)
    {
        ...
        try
        {
            ...
            sql = "SELECT Distinct RIGHT('00000'+ CONVERT(VARCHAR,Accno),6) AS ACCNO, " +
                ...
                "  FROM AVS_ACC A  INNER JOIN MASTER M ON M.CUSTNO=A.CUSTNO " +
                " WHERE A.SUBGLCODE='" + AGNO + "'  AND A.GLCODE='2' AND A.BRCD='" + BRCD + "' AND A.ACC_STATUS=1";
    ...
    public DataTable GetGSTData(string BRCD, string PRDCD)
    {
        try
        {
            sql = "Select BRCD,EFFECTDATE,GST,PRDCD,CGST,CGSTPRDCD,SGST,SGSTPRDCD from Dbo.AVS5057 " +
                    " where Brcd='" + BRCD + "'  " +
                    " and PRDCD='" + PRDCD + "'  " +
                    " and STAGE<>'1004'";
            DT = new System.Data.DataTable();
    ...
    public void DelAllRecTable(string BrCode, string EDate, string Mid)
    {
        try
        {
            sql = "Delete From TEMPBATCH Where BrCd = '" + BrCode + "' And MID_DATE = '" + conn.ConvertDate(EDate).ToString() + "' And Mid = '" + Mid + "'" +
    ...
    public DataTable GetLoanTotalAmount(string BrCode, string PrCode, string AccNo, string EDate, string FL = "CL")
    {
        try
        {
    ...
    public int InsertCollection_R(string BRCD, string MID, string EDT,string Glcode,string Subglcode,string Accno,string TrxAmt,string TrxBal,string TrxDate,string Mid)
    {
        try
        {
            sql = "insert into Collection_R(Brcd,Entrydate,Glcode,Subglcode,Accno,TrxAmt,TrxBalance,TrxDate,Mid,Systemdate) " +
                   " values ('" + BRCD + "','" + conn.ConvertDate(EDT) + "','" + Glcode + "','" + Subglcode + "','" + Accno + "','" + TrxAmt + "','" + TrxBal + "','" + conn.ConvertDate(TrxDate) + "','" + Mid + "',Getdate()) ";
    ...
    public DataTable GetInvalidAccount(string Brcd,string Edt,string Subglcode)
    {
        try
        {
            sql = "Select Row_Number() over (order by BRCD) ID,'' NAME,Brcd,Subglcode,ACCNO,TrxAmt as AMOUNT,'Acc not Found' Remark from Collection_R A " +
                " where A.Brcd='" + Brcd + "' " +
                " and A.Entrydate='" + conn.ConvertDate(Edt) + "' " +
                " and A.SUBGLCODE='" + Subglcode + "'" +
                " and Concat(A.Brcd,A.Subglcode,A.Accno) not in (Select Concat(Brcd,Subglcode,Accno) from Avs_ACC B " +
                                         " where B.SUBGLCODE='" + Subglcode + "' " +
                                         " and B.BRCD='" + Brcd + "' " +
                                         " and B.ACC_STATUS<>'3') ";
            DT = conn.GetDatatable(sql);
        }
    ...
    public int InsertData_Aftekr(string AG, string BRCD, string MID, string EDT) // new Logic 20-02-2018
    {
        try
        {
            sql = "insert into AFTEKR(ENTRYDATE,TRANSAMT,AGENTCODE,BRCD,ACNO,STAGE,MID,SYSTEMDATE)" +
                "select ENTRYDATE,TRXAMT,SUBGLCODE,BRCD,ACCNO,'1003',MID,GETDATE() from COLLECTION_R " +
                " where GLCODE='2' and SUBGLCODE in ('" + AG + "')  " +
                " and BRCD='" + BRCD + "' and Entrydate='" + conn.ConvertDate(EDT) + "' " +
                 "and CONCAT(Brcd,Subglcode,accno) in (Select CONCAT(Brcd,Subglcode,accno) from Avs_Acc " +
                                    " Where brcd='" + BRCD + "' " +
                                    " and Subglcode='" + AG + "' " +
                                    " and ACC_STATUS<>'3' and STAGE<>'1004')  ";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int UpdateAFTData(string AG, string EDT, string PDT, string AC, string BRCD, string AMT, string TDT, string FL)
    {
        try
        {
            //PDT = PDT.Substring(0, 11).ToString();
            ...
            else if (FL == "CHGAMT")
            {
                ...
                sql = "update AFTEKR set TRANSAMT=isnull(TRANSAMT,0)+'" + AMT + "',TRANSACTIONDATE='" + conn.ConvertDate(EDT) + "',POSTINGDATE='" + conn.ConvertDate(EDT) + "' where ENTRYDATE='" + conn.ConvertDate(EDT) + "' and AGENTCODE='" + AG.Trim() + "' AND  BRCD ='" + BRCD.Trim() + "' AND ACNO='" + AC.Trim() + "'";
            }
    ...
    public DataTable getDetails(string brcd, string subgl, string accno)////Added by ankita on 19/06/2017 to display lien details
    {
        ...
        try
        {
            sql = "SELECT LOANGLCODE,LOANACCNO,LOANAMT FROM AVS_LIENMARKDETAILS WHERE DEPOSITGLCODE='" + subgl + "' AND DEPOSITACCNO='" + accno + "' AND BRCD='" + brcd + "' and status=1 and stage<>1004";
            DT = conn.GetDatatable(sql);
        }
        catch (Exception ex)
        {
    ...
    public DataTable UpdateAvsACC1(string AgentCode, string BRCD, string EDAT, string Flag, string BankCode)
    {
        DataTable DT = new DataTable();
        try
        {
            if (BankCode == "1007")
                sql = "exec ISP_AVS0119 '" + AgentCode + "','" + BRCD + "','" + conn.ConvertDate(EDAT) + "','" + Flag + "'";
            else
                sql = "exec ISP_AVS0051 '" + AgentCode + "','" + BRCD + "','" + conn.ConvertDate(EDAT) + "','" + Flag + "'";
            DT = conn.GetDatatable(sql);
        }
    ...
    public void InsertAvsACC1(DataTable dt, string BCODE, string Flag)
    {
        try
        {
            if (Flag == "1")
    ...
    public DataTable getAllVCRData1(string AgentCode, string BRCD, string BankCD, string EDAT, string Flag)
    {
        DataTable dt = new DataTable();
        try
        {
    ...
    public void DeleteAVSACC1(string AgentCode, string BRCD, string BCode, string Flag)
    {
        try
        {
            if (Flag == "2")
    ...
    //Exec Isp_AVS0118 @Flag='Post',@Brcd='11',@Edt='2018-01-24',@DebitGlcode='2',@DebitSubglcode='6',@DebitAccno='1210',@DebitAmt='63680.00',@Mid='100',@F1='143131345'
    public int PostData_SP(string FL, string SFL, string EDT, string SETNO, string DebitGl, string DebitSubgl, string DebitAccno, string DebitAmt, string MID, string BRCD,string NARR)
    {
        try
        {
            EntryMid = Ecry.GetMK(MID.ToString());

            sql = "Exec Isp_AVS0118 @Flag='" + FL + "',@SFlag='" + SFL + "',@Setno='" + SETNO + "',@Brcd='" + BRCD + "',@Edt='" + conn.ConvertDate(EDT) + "',@DebitGlcode='" + DebitGl + "',@DebitSubglcode='" + DebitSubgl + "',@DebitAccno='" + DebitAccno + "',@DebitAmt='" + DebitAmt + "',@Mid='" + MID + "',@F1='" + EntryMid + "',@Narration='" + NARR + "'";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public string CheckData_SP(string FL, string EDT, string DebitAccno, string MID, string BRCD)
    {
        try
        {

            sql = "Exec Isp_AVS0118 @Flag='" + FL + "',@Brcd='" + BRCD + "',@Edt='" + conn.ConvertDate(EDT) + "',@DebitAccno='" + DebitAccno + "',@Mid='" + MID + "'";
            sql = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
    ...
    public int UpdateStage(string AgentCode, string BRCD, string BankCD, string EDAT, string Flag)
    {
        try
        {
            sql = "Exec ISP_AVS0052 '" + AgentCode + "','" + BRCD + "','" + BankCD + "','" + conn.ConvertDate(EDAT) + "','" + Flag + "'";
    ...
    //  End added by amol on 19/10/2018
    //  Added by amol on 19/10/2018 (Insert Data and Update mobile data status)
    public int InsertBulk(DataTable DT)
    {
        try
        {
            using (SqlConnection con = new SqlConnection(conn.DbName()))
