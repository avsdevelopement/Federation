...
public class ClsMultiVoucherr
{
    ...
    public int InsertIntoTable(string BrCode, string RefBrCode, string CustNo, string GlCode, string SubGlCode, string AccNo, string CustName, string Amount, string TrxType, string Activity, string PmtMode, string Perticulars, string Perticulars2, string ChkNo, string ChkDate, string EDate, string Mid)
    {
        try
        {
            sql = "Insert Into Avs_TempABBMultiTrf (BrCd, RefBrCd, CustNo, CustName, GlCode, SubGlCode, AccNo, Particulars, Particulars2, Amount, TrxType, Activity, PmtMode, RefId, InstNo, InstDate, EntryDate, SystemDate, Mid) " +
                  "VALUES ('" + BrCode + "','" + RefBrCode + "','" + CustNo + "','" + CustName + "', '" + GlCode + "','" + SubGlCode + "','" + AccNo + "','" + Perticulars + "','" + Perticulars2 + "','" + Amount + "','" + TrxType + "','" + Activity + "','" + PmtMode + "','0','" + ChkNo + "','" + ChkDate + "', '" + conn.ConvertDate(EDate).ToString() + "', GetDate(), '" + Mid + "')";
            Result = conn.sExecuteQuery(sql);
        }
        catch (Exception Ex)
        {
    ...
    public string GetOrgBrCode(string BrCode, string EDate, string Mid)
    {
        try
        {
            sql = "Select Top 1 IsNull(BrCd, '') From Avs_TempABBMultiTrf " +
                  "Where RefBrCd = '" + BrCode + "' And EntryDate = '" + conn.ConvertDate(EDate).ToString() + "' And Mid = '" + Mid + "' And BrCd <> '" + BrCode + "'";
    ...
    public DataTable GetDRTransDetails(string BrCode, string EDate, string Mid)
    {
        try
        {
            sql = "SELECT ID, BrCd, CustNo, CustName, GlCode, SubGlCode, AccNo, Particulars, Particulars2, Amount, TrxType, Activity, PmtMode, "+
                  "RefId, InstNo, Convert(VarChar(10), InstDate, 103) As InstDate, EntryDate, Mid From Avs_TempABBMultiTrf "+
                  "Where RefBrCd = '" + BrCode + "' And EntryDate = '" + conn.ConvertDate(EDate).ToString() + "' And Mid = '" + Mid + "' And TrxType = '2' "+
                  "Order By SystemDate ";
            DT = new DataTable();
            DT = conn.GetDatatable(sql);
        }
    ...
    public DataTable GetCRTransDetails(string BrCode, string EDate, string Mid)
    {
        try
        {
            sql = "SELECT ID, BrCd, CustNo, CustName, GlCode, SubGlCode, AccNo, Particulars, Particulars2, Amount, TrxType, Activity, PmtMode, "+
                  "RefId, InstNo, Convert(VarChar(10), InstDate, 103) As InstDate, EntryDate, Mid From Avs_TempABBMultiTrf "+
                  "Where RefBrCd = '" + BrCode + "' And EntryDate = '" + conn.ConvertDate(EDate).ToString() + "' And Mid = '" + Mid + "' And TrxType = '1' "+
                  "Order By SystemDate ";
            DT = new DataTable();
            DT = conn.GetDatatable(sql);
        }
    ...
    public double CreditBrCount(string BrCode, string EDate, string Mid)
    {
        try
        {
            sql = "Select Count(Distinct BrCd) From Avs_TempABBMultiTrf Where RefBrCd = '" + BrCode + "' And EntryDate = '" + conn.ConvertDate(EDate).ToString() + "' And Mid = '" + Mid + "' And TrxType = '1'";
            sResult = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
    ...
    public double DebitBrCount(string BrCode, string EDate, string Mid)
    {
        try
        {
            sql = "Select Count(Distinct BrCd) From Avs_TempABBMultiTrf Where RefBrCd = '" + BrCode + "' And EntryDate = '" + conn.ConvertDate(EDate).ToString() + "' And Mid = '" + Mid + "' And TrxType = '2'";
            sResult = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
