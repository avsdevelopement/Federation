...
public class ClsLoanLimit
{
    ...
    public DataTable GetProdDetails(string BrCode, string PrCode)
    {
        ...
        try
        {
            sql = "Select G.GlCode, G.SubGlCode, G.GlName, IsNull(G.UnOperate, '0') As UnOperate, ISNULL(G.IntAccYN, 'N') As IntAccYN, L.LoanCategory, L.IntCalType, L.Int_App " +
                  "From GlMast G With(NoLock) " +
                  "Inner Join LoanGl L With(NoLock) On G.BrCd = L.BrCd And G.SubGlCode = L.LoanGlCode " +
                  "Where G.BrCd = '" + BrCode + "' And G.SubGlCode = '" + PrCode + "' ";
    ...
    public int InsertLoan(string CUSTNO, string CUSTACCNO, string LOANGLCODE, string AppNo, string LIMIT, string SANSSIONDATE, string INTRATE, string INSTDATE, string INSTALLMENT,
        string PERIOD, string DUEDATE, string PENAL, string LMSTATUS, string BONDNO, string MID, string BRCD, string FL, string INSTTYPE, string SancAutho, string RecommAutho,
        string Remark, string Equated, string IntFund, string Frequency, string GraceDInst, string GraceDIntr, string PLRLink, string MoraPeriod, string EffectDate, string secured, string EDate)
    {
        try
        {
            bool isUpdate = false;
            ...
            try
            {
                DateTime WorkDate = Convert.ToDateTime(conn.ConvertDate(EDate.ToString()).ToString()).Date; ;
                ...
                if (FL == "1002")
                {
                    ...
                    sql = "UPDATE AVS1015 SET LMSTATUS=99 FROM AVS1015 WHERE CUSTNO='" + CUSTNO + "' AND ACCNO='" + CUSTACCNO + "' AND SUBGL='" + LOANGLCODE + "' AND BRCD='" + BRCD + "' AND LMSTATUS<>99 AND NEWSUBS<>" + INSTALLMENT;
                    Result = conn.sExecuteQuery(sql);

                    sql = "INSERT INTO AVS1015(BRCD,GLCODE,SUBGL,CUSTNO,ACCNO,OLDSUBS,NEWSUBS,EFFECTDATE,LMSTATUS,STAGE,RTIME,MID" +
                        ",CID,VID,PCMAC,YEAR,MONTH,REASON,SYSTEMDATE)" +
                       "SELECT TOP 1 BRCD,(select top 1 GLCODE from glmast where brcd=BRCD and subglcode=LOANGLCODE),LOANGLCODE,CUSTNO,CUSTACCNO,INSTALLMENT," + INSTALLMENT + ",'" + conn.ConvertDate(EDate) + "',LMSTATUS,1001,GETDATE()," + MID +
                       ",CID,null,'" + conn.PCNAME() + "',null,null,'" + Remark + "',getdate() " +
                       "FROM LOANINFO WHERE CUSTNO='" + CUSTNO + "' AND CUSTACCNO='" + CUSTACCNO + "' AND LOANGLCODE='" + LOANGLCODE + "' AND BRCD='" + BRCD + "' AND LMSTATUS<>99 AND INSTALLMENT<>" + INSTALLMENT + "  ";
                    Result = conn.sExecuteQuery(sql);
                ...
                sql = "INSERT INTO LOANINFO(CUSTNO, CUSTACCNO, LOANGLCODE, APPLINO, LIMIT, SANSSIONDATE, DRAW_POWER, INTRATE, INSTALLMENT, INSTDATE, PERIOD, DUEDATE, PENAL, LMSTATUS, BONDNO, MID, PCMAC, STAGE, BRCD, DISYN, INSTTYPE, SancAutho, RecommAutho, Remark, Equated, IntFund, Frequency, GraceDInst, GraceDIntr, PLRLink, MoraPeriod, EffectDate, MOD_DATE) " +
                      ...
                      "'" + MID + "', '" + conn.PCNAME() + "', '1001', '" + BRCD + "','1','" + INSTTYPE + "', '" + SancAutho + "', '" + RecommAutho + "', '" + Remark + "', '" + Equated + "', '" + IntFund + "','" + Frequency + "','" + GraceDInst + "', '" + GraceDIntr + "', '" + PLRLink + "', '" + MoraPeriod + "', '" + conn.ConvertDate(EffectDate) + "', '" + conn.ConvertDate(EDate) + "')";//+ DateTime.Now.ToString("yyyy-MM-dd") + 
                Result = conn.sExecuteQuery(sql);
                if (Result > 0)
                {
            ...
            if (isUpdate == true)
            {
                sql = "INSERT INTO AVS500(BRCD,ENTRYDATE,VID,SYSTEMDATE,PCMAC,IP,ACTIVITY,OLDVALUE,NEWVALUE,MID,ACTNO)" +
                         "values('" + BRCD + "','" + conn.ConvertDate(EDate.ToString()) + "','" + MID + "',GETDATE(),NULL,NULL,'UPDATE LOANINFO _ACCNO_" + CUSTACCNO + "_SUBGLCODE_" + LOANGLCODE +
                         "',NULL,NULL,'" + MID + "',NULL)";
                Result = conn.sExecuteQuery(sql);
            }
            else
            {
                sql = "INSERT INTO AVS500(BRCD,ENTRYDATE,VID,SYSTEMDATE,PCMAC,IP,ACTIVITY,OLDVALUE,NEWVALUE,MID,ACTNO)" +
                        "values('" + BRCD + "','" + conn.ConvertDate(EDate.ToString()) + "','" + MID + "',GETDATE(),NULL,NULL,'INSERT LOANINFO _ACCNO_" + CUSTACCNO + "_SUBGLCODE_" + LOANGLCODE +
                        "',NULL,NULL,'" + MID + "',NULL)";
                Result = conn.sExecuteQuery(sql);
            }
        }
    ...
    public int InsertLoanApp(string CUSTNO, string CUSTACCNO, string LOANGLCODE, string AppNo, string LIMIT, string SANSSIONDATE, string INTRATE, string INSTDATE, string INSTALLMENT,
        string PERIOD, string DUEDATE, string PENAL, string LMSTATUS, string BONDNO, string MID, string BRCD, string FL, string INSTTYPE, string SancAutho, string RecommAutho,
        string Remark, string Equated, string IntFund, string Frequency, string GraceDInst, string GraceDIntr, string PLRLink, string MoraPeriod, string EffectDate)
    {
        try
        {
            string lastintdt = "";
            ...
            //  Create new loan info
            sql = "INSERT INTO LOANINFO(CUSTNO, CUSTACCNO, LOANGLCODE, APPLINO, LIMIT, SANSSIONDATE, DRAW_POWER, INTRATE, INSTALLMENT, INSTDATE, PERIOD, DUEDATE, PENAL, LMSTATUS, BONDNO, MID, PCMAC, STAGE, BRCD, DISYN, INSTTYPE, SancAutho, RecommAutho, Remark, Equated, IntFund, Frequency, GraceDInst, GraceDIntr, PLRLink, MoraPeriod, EffectDate, MOD_DATE, LastINTDate, LoanPurpose) " +
                  "VALUES('" + CUSTNO + "', '" + CUSTACCNO + "', '" + LOANGLCODE + "', '" + AppNo + "', '" + LIMIT + "','" + conn.ConvertDate(SANSSIONDATE) + "', '" + LIMIT + "', '" + INTRATE + "', '" + INSTALLMENT + "','" + conn.ConvertDate(INSTDATE) + "', '" + PERIOD + "', '" + conn.ConvertDate(DUEDATE) + "', '" + PENAL + "', '" + LMSTATUS + "', '" + BONDNO + "', " +
                  "'" + MID + "', '" + conn.PCNAME() + "', '1001', '" + BRCD + "','1','" + INSTTYPE + "', '" + SancAutho + "', '" + RecommAutho + "', '" + Remark + "', '" + Equated + "', '" + IntFund + "','" + Frequency + "','" + GraceDInst + "', '" + GraceDIntr + "', '" + PLRLink + "', '" + MoraPeriod + "', '" + conn.ConvertDate(EffectDate) + "', " +
    ...
    public int Lon_OPR(string CUSTACCNO, string CUSTNO, string MID, string LOANGLCODE, string FL, string BRCD, string EDate)
    {
        try
        {
            if (FL == "DELETE")
            {
                sql = "UPDATE LOANINFO SET LMSTATUS = '99', STAGE = 1004, Mod_Date = '" + conn.ConvertDate(EDate).ToString() + "', Vid = '" + MID + "' " +
                      "WHERE BRCD = '" + BRCD + "' AND LOANGLCODE = '" + LOANGLCODE + "' AND CUSTACCNO = '" + CUSTACCNO + "' AND LMSTATUS = 1";
                Result = conn.sExecuteQuery(sql);

                //sql = "UPDATE AVS1015 SET LMSTATUS = '99', STAGE = 1004,Vid = '" + MID + "',SYSTEMDATE=GETDATE() FROM AVS1015 WHERE CUSTNO='" + CUSTNO + "' AND ACCNO='" + CUSTACCNO + "' AND SUBGL='" + LOANGLCODE + "' AND BRCD='" + BRCD + "' AND LMSTATUS=1 AND STAGE not in(1004)";
                //Result = conn.sExecuteQuery(sql);
                ...
                sql = "Update AVS_LNBasic set CID='" + MID + "',STAGE='1004' where PRDCODE='" + LOANGLCODE + "' and ACCTNO='" + CUSTACCNO + "' And BRCD='" + BRCD + "'";
                Result = conn.sExecuteQuery(sql);
            }
            else if (FL == "AUTHO")
            {
                sql = "UPDATE LOANINFO SET STAGE = 1003, Mod_Date = '" + conn.ConvertDate(EDate).ToString() + "', Vid = '" + MID + "' " +
                      "WHERE BRCD = '" + BRCD + "' AND LOANGLCODE = '" + LOANGLCODE + "' AND CUSTACCNO = '" + CUSTACCNO + "' AND LMSTATUS = 1 And Stage <> '1004'";
                Result = conn.sExecuteQuery(sql);

                //sql = "UPDATE AVS1015 SET STAGE = 1003,Vid = '" + MID + "',SYSTEMDATE=GETDATE() FROM AVS1015 WHERE CUSTNO='" + CUSTNO + "' AND ACCNO='" + CUSTACCNO + "' AND SUBGL='" + LOANGLCODE + "' AND BRCD='" + BRCD + "' AND LMSTATUS=1 AND STAGE not in(1004,1003)";
                //Result = conn.sExecuteQuery(sql);
                sql = "Update AVS_LNBasic set VID='" + MID + "', STAGE='1003' where PRDCODE='" + LOANGLCODE + "' and ACCTNO='" + CUSTACCNO + "' And BRCD='" + BRCD + "'";
                Result = conn.sExecuteQuery(sql);
            }
            else if (FL == "VIEW")
            {
                sql = "SELECT * FROM LOANINFO WHERE BRCD='" + BRCD + "' And LOANGLCODE='" + LOANGLCODE + "' AND CUSTACCNO='" + CUSTACCNO + "' AND LMSTATUS <> 99 And Stage <> 1004";
                Result = conn.sExecuteQuery(sql);
            }
        }
        catch (Exception Ex)
    ...
    public string GetLimitROI(string brcd, string subgl, string EDT)
    {
        try
        {
            sql = "select top (1)(isnull(convert(varchar(10),ROI),'0')+'_'+isnull(convert(varchar(10),PENALINT),'0')) As ROI from avs5028 where EffectDate<='" + conn.ConvertDate(EDT) + "' and subglcode='" + subgl + "' order by EffectDate desc";
            sResult = conn.sExecuteScalar(sql);
        }
        catch (Exception ex)
        {
    ...
    public string GetLoanLimStage(string BRCD, string AccNo, string LoanGlCode)
    {
        try
        {
            sql = "SELECT STAGE FROM LOANINFO " +
                  "WHERE BRCD='" + BRCD + "' And LOANGLCODE='" + LoanGlCode + "' And CUSTACCNO='" + AccNo + "' And LmStatus <> '99' And Stage <> '1004' ";
            sResult = conn.sExecuteScalar(sql);
        }
        catch (Exception Ex)
        {
    ...
    public string GetStageMid(string BRCD, string LoanGlCode, string AccNo)
    {
        try
        {
            sql = "SELECT MID FROM LOANINFO " +
                  "WHERE BRCD='" + BRCD + "' And LOANGLCODE='" + LoanGlCode + "' And CUSTACCNO='" + AccNo + "' And LmStatus <> '99' And Stage <> '1004' ";
            sResult = conn.sExecuteScalar(sql);
        }
        catch (Exception ex)
