...
public class CLsShareCertificate
{
    ...
    public int BindData(GridView Gview, string BrCode, string accno)
    {
        ...
        try
        {
            sql = "select s.CustNo,s.CustAccNo,m.custname,s.sharefrom,s.shareto,s.totalshares,s.sharesvalue,s.totalshareamt,s.cert_no,s.cert_issue1stdate,convert(varchar(10),s.OPENDATE,103) OPENDATE from SHARESINFO s LEFT join master m on s.custno=m.custno Left Join Avs_Acc A On A.Accno = S.CustAccno And A.Custno = S.Custno And A.Subglcode='4' where A.Accno='" + accno + "'";
    ...
    {
        sql = "select m.custname from SHARESINFO s LEFT join master m on s.custno=m.custno Left Join Avs_Acc A On A.Accno = S.CustAccno And A.Custno = S.Custno And A.Subglcode='4' where A.Accno='" + accno + "'";
        Result = conn.sExecuteScalar(sql);
        return Result;
    }
    public DataTable GetShareCerti(string BRCD, string AccNo,string Entrydate,string MID)
    {
        sql = "EXEC Isp_AVS0001 '" + BRCD + "','" + AccNo + "','"+conn.ConvertDate(Entrydate)+"','"+MID+"'";
        DT = conn.GetDatatable(sql);
        return DT;
    }
    public DataTable ShareCertiPrintingAddshr(string BRCD, string AccNo, string cerno, string Entrydate, string MID)
    {
        sql = "EXEC ISP_AVS0111 '" + BRCD + "','" + AccNo + "','" + cerno + "','" + conn.ConvertDate(Entrydate) + "','" + MID + "'";
        DT = conn.GetDatatable(sql);
        return DT;
    }
    public DataTable GetShareCerti_Palghar(string BRCD, string AccNo)
    {
        sql = "EXEC Isp_AVS0059 @Brcd='" + BRCD + "',@MemberNo='" + AccNo + "'";
        DT = conn.GetDatatable(sql);
        return DT;
    }
    public DataTable GetShareCerti_ShivSamarth(string BRCD, string AccNo)
    {
        try
        {
            sql = "EXEC Isp_AVS0070 @brcd='" + BRCD + "',@memno='" + AccNo + "'";
            DT = conn.GetDatatable(sql);
        }
        catch (Exception Ex)
        {
    ...
    {
        sql = "SELECT isnull(MAX(CUSTACCNO),0)+1 FROM AVS_InvAccountMaster WHERE SUBGLCODE='" + subglcode + "' AND BRCD='" + brcd + "'";
        string Result = conn.sExecuteScalar(sql);
        return Result;
    }
    ...
    public DataTable GetShareCertiAjinkya(string BRCD, string AccNo)
    {
        try
        {
            sql = "EXEC ISP_AVS0162 @Brcd='" + BRCD + "',@Accno='" + AccNo + "'";
            DT = conn.GetDatatable(sql);
        }
        catch (Exception Ex)
        {
