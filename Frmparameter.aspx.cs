...
public partial class Frmparameter : System.Web.UI.Page
{
    ...
    protected void Submit_Click(object sender, EventArgs e)
    {
        ...
        {
            Result = OP.Insertpara("INSERT", "0", TxtEffectDT.Text, Session["BRCD"].ToString(), TxtOWGType.Text, TxtEntryDT.Text, TxtFunDT.Text, "1", TxtCLGNO.Text, TxtCLGName.Text, TxtRTGLNo.Text, TxtRTGLName.Text,Session["MID"].ToString(),"OUT");
            BindGrid();
            ClearData();
            WebMsgBox.Show("Record Added Successfully........!!", this.Page);
            FL = "Insert";//Dhanya 14/09/2017
            string Res = CLM.LOGDETAILS(FL, Session["BRCD"].ToString(), Session["MID"].ToString(), "Parameter_Add_" + Session["LOGINCODE"].ToString() + "", "00", Session["MID"].ToString());
            return;
        }
        else if (ViewState["Flag"].ToString() == "ED")
        {
            ...
            Result = 0;//OP.Insertpara("UPDATE", ViewState["strnumId"].ToString(), TxtEffectDT.Text, Session["BRCD"].ToString(), TxtOWGType.Text, TxtEntryDT.Text, TxtFunDT.Text, "1", TxtCLGNO.Text, TxtCLGName.Text, TxtRTGLNo.Text, TxtRTGLName.Text,"OUT");
            BindGrid();
            ClearData();
            WebMsgBox.Show("Record Modify Successfully........!!", this.Page);
            FL = "Insert";//Dhanya 14/09/2017
            string Res = CLM.LOGDETAILS(FL, Session["BRCD"].ToString(), Session["MID"].ToString(), "Parameter_Mod_" + Session["LOGINCODE"].ToString() + "", "00", Session["MID"].ToString());
            return;
        }
        else if (ViewState["Flag"].ToString() == "D")
        {
            ...
            FL = "Insert";//Dhanya 14/09/2017
            string Res = CLM.LOGDETAILS(FL, Session["BRCD"].ToString(), Session["MID"].ToString(), "Parameter_Del_" + Session["LOGINCODE"].ToString() + "", "00", Session["MID"].ToString());
            return;
        }
    }
