...
public partial class FrmOutAuthDo : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            dt = maincode.GetFormData(setNo, scrollNo, Convert.ToInt32(Session["BRCD"]), Session["EntryDate"].ToString(), "O");
        }

        if (ViewState["op"] == "return" || ViewState["op"] == "returnauth")
        {
            dt = maincode2.GetFormDatareturn(setNo, scrollNo, Convert.ToInt32(Session["BRCD"]), Session["EntryDate"].ToString(), "O");
        }
    ...
    protected void btnAuthorize_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                result = maincode.Authorize(1003, setNo, scrollNo, Convert.ToInt32(Session["BRCD"]), Session["MID"].ToString(), Session["EntryDate"].ToString(), "O");
                if (result > 0)
                {
