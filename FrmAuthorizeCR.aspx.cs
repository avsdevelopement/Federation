...
public partial class FrmAuthorizeCR : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (PayMast != null)
            {
                ...
                if (DT.Rows.Count > 0)
                {
                    ...
                    txtCrAmount.Text = DT.Rows[0]["AMOUNT"].ToString();
                    ...
                    sResult = AC.GetVoucherCount(Session["BRCD"].ToString(), Session["EntryDate"].ToString(), SetNo);
                    if (Convert.ToInt32(sResult) > 2)
                    {
                        rbtnExistSet.Checked = true;
                        DivExistSet.Visible = true;
                        txtExistSetNo.Text = SetNo.ToString();
                    }

                    txtCrAmount.ForeColor = System.Drawing.Color.Red;
    ...
    protected void btnAuthorize_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (Convert.ToDouble(txtCrAmount.Text.ToString()) == Convert.ToDouble(txtPassAmount.Text.ToString() == "" ? "0" : txtPassAmount.Text.ToString()))
                {
                    ...
                    if (Result > 0)
                    {
                      ...
                      //  Message = "Your a/c no " + Convert.ToInt32(txtAccNo.Text.Trim().ToString()) + " has been credited with Rs " + Convert.ToDouble(txtPassAmount.Text.Trim().ToString()) + ". Available a/c balance is Rs " + Balance + ".";
                        Message = "Your a/c no " + Convert.ToInt32(txtAccNo.Text.Trim().ToString()) + " has been credited with Rs " + Convert.ToDouble(txtPassAmount.Text.Trim().ToString()) + " On Date " + Session["EntryDate"].ToString() + ". Available a/c balance is Rs " + Balance + ".";
