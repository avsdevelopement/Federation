...
public partial class FrmAVS5065 : System.Web.UI.Page
{
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string Res = CLM.LOGDETAILS(FL, Session["BRCD"].ToString(), Session["MID"].ToString(), "LoanIntCertf1 _Rpt_" + Session["LOGINCODE"].ToString() + "", "00", Session["MID"].ToString());
            if (Rdb_No.SelectedValue == "1")
            {
                string redirectURL = "FrmRView.aspx?BRCD=" + Session["BRCD"].ToString() + "&LOANGL=" + txtProdCode.Text + "&Edate=" + Session["ENTRYDATE"].ToString() + "&ACCNO=" + txtaccno.Text + "&FDate=" + txtFDate.Text + "&TDate=" + txtTDate.Text + "&rptname=RptAVS5065.rdlc";
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "popup", "window.open('" + redirectURL + "','_blank')", true);
            }
            else if  (Rdb_No.SelectedValue=="2")
            {
                string redirectURL = "FrmRView.aspx?BRCD=" + Session["BRCD"].ToString() + "&LOANGL=" + txtProdCode.Text + "&Edate=" + Session["ENTRYDATE"].ToString() + "&ACCNO=" + txtaccno.Text + "&FDate=" + txtFDate.Text + "&TDate=" + txtTDate.Text + "&rptname=RptLoanInthead.rdlc";
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "popup", "window.open('" + redirectURL + "','_blank')", true);
            }
        }
        catch (Exception Ex)
